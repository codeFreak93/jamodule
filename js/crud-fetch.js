...
const deleteUser = (idUser) => {
    fetch(`https://koders1gpython-default-rtdb.firebaseio.com/juandedios/users/${idUser}.json`, {
        method: 'DELETE'
    })
    .then(response => {
...
const readUsers = () => {
    ...
    fetch(`https://koders1gpython-default-rtdb.firebaseio.com/juandedios/users/.json`, options )
    .then(response => {
        return response.json()
    })
    .then((users) => {  
...
const createUserFetch = (data) => {
    // create
    fetch(`https://koders1gpython-default-rtdb.firebaseio.com/juandedios/users/.json`, {
        method: 'POST',
        headers: {
...
let btn__send__form = document.querySelector('.btn_send_form')
if (btn__send__form){
    btn__send__form.addEventListener('click', event =>{
